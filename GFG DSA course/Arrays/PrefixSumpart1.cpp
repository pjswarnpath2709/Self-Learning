...
ll getSum(vll vec, int leftidx, int rightidx)
{
    int ans = (leftidx>0)?vec[rightidx] - vec[leftidx]:vec[rightidx];
    return ans;
}
int main()
{
    ...
    in_vector(vec, n);
    int sum = 0;
    // calculated the prefix sum of the array
    for (int i = 0; i < vec.size(); i++)
    {
        sum += vec[i];
        vec[i] = sum;
