...
int longestSubstringWithKuniqueCharacters(string str, int k)
{
    int n = str.size();
    int ans = 0;
    int j = 0;
    ...
    while (j < n)
    {
        ...
        mp[str[j]]++;
        // while condition is not fullfilled
        if (mp.size() < k)
        {
            j++;
        }
        else if (mp.size() == k)
        {
            // we got the answer
            ans = max(ans, j + i - 1);
            // increase the size intentionally
            j++;
        }
        else
        {
            // remove the calculations of i
            while (mp.size() >= k)
            {
                if (mp[str[i]] == 0)
                {
            ...
            // increase the window size intentionally
            j++;
        }
    }
    return ans;
...
int main()
{
    ...
    int k;
    cin >> k;
    cout << longestSubstringWithKuniqueCharacters(str, k) << "\n";
    return 0;
}